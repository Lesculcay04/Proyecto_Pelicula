...
export default function Tab1() {
  ...
  const fetchMovies = async () => {
    try {
      const response = await fetch('http:// 10.10.3.39:8080/film');
  ...
  const handleEdit = async (filmId: number, updatedData: MovieData) => {
    try {
      await axios.put(`http:// 10.10.3.39:8080/film/${filmId}`, updatedData);
  ...
  const handleDelete = async (id: number) => {
    try {
      await axios.delete(`http:// 10.10.3.39:8080/film/${id}`);
